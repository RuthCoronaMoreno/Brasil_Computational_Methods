...
abline(mod_cat, col = "red", lwd = 2) #It's only useful with object type "lm"


## ----lm-ggplot----------------------------------------------------------------
ggplot(data = cat, mapping = aes(x = tannin, y = growth)) +
  geom_point() +
  geom_smooth(method = lm, level=0.50) + #Add confidence interval // we have several method to use
  theme_classic()
...
## fitted values
predict(mod_cat) #Predicted values
cat$fitted <- predict(mod_cat) #Creating a new column in my data "cat" called fitted
